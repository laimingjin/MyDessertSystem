...
<body>
   ...
   <!------------End Header ------------>
    ...
    %>
    <form action="/MyDessertSystem/paybycashAndCredit?account=<%=account %>&planIDs=<%=planIDs %>&productIDs=<%=productIDs %>&productNames=<%= productNames%>&quantities=<%=quantities %>&prices=<%= prices%>&totalPrice_after=<%=totalPrice_after%>&credit=<%=credit %>" method="post">

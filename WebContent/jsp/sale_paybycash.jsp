...
<body>
   ...
   <!------------End Header ------------>
    ...
    %>
    <form action="/MyDessertSystem/sale_paybycash?member_account=<%=member_account %>&waiter_account=<%=waiter_account %>&planIDs=<%=planIDs %>&productIDs=<%=productIDs %>&productNames=<%= productNames%>&quantities=<%=quantities %>&prices=<%= prices%>&totalPrice_after=<%=totalPrice_after%>" method="post">
